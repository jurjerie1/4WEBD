                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <img src={`${apiUrl}/EventService${event.image}`}
                                 alt={event.name} className="w-full h-96 object-cover rounded-lg shadow-md"/>
                        </div>
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                <h1 className="text-3xl font-bold text-indigo-600">{event.name}</h1>
                                {totalPrice && (
                                    <p className="text-gray-500">Prix total: <span
                                        className="font-semibold text-gray-900">{totalPrice}€</span></p>
                                )}
                            </div>
                            <p className="text-gray-700 mb-4">{event.description}</p>
                            <p className="text-gray-700 mb-4">Localisation: <span
                                className="font-semibold text-gray-900">{event.location}€</span></p>
                            <p className="block text-gray-700 mb-4">Le <span
                                className="font-semibold text-gray-900">{new Date(event.date).toLocaleDateString('fr-FR')}</span> à <span
                                className="font-semibold text-gray-900">{new Date(event.date).toLocaleTimeString('fr-FR', {
                                hour: '2-digit',
                                minute: '2-digit'
                            })}</span></p>
                            ...
                            <button
                                ...
                                onClick={handleBooking}>Réserver
                            </button>
                        </div>
                    </div>
                </div>
